...
Integrator.prototype.step = function( tLimit ) {
  ...
  while(true) {
    ...
    if( error < this.tol || dtLimitReached ) {
      // Success! Exit:
      break
    }
    ...
    // Modification by Peter Schmidt-Nielsen. Also limit the maxmimum step.
    // If stepsize too large, finish off by taking the currently proposed step and logging a warning:
    if( this.dtMaxMag !== undefined && Math.abs(thisDt) > this.dtMaxMag ) {
      thisDt = this.dtMaxMag * (thisDt > 0 ? 1 : -1);
      this.__log('step', 'maximum stepsize reached.')
      dtLimitReached = true
    }
  }
  ...
  // Apply this update:
  this._update(thisDt)
  this.stepSizeActuallyTaken = thisDt;
  ...
  nextDt = maxMag(this.dt / this.maxDecreaseFactor, minMag( this.dt * this.maxIncreaseFactor, nextDt ));

  // Modification by Peter Schmidt-Nielsen. Limit the nextDt just as we limit thisDt.
  if (this.dtMinMag !== undefined && Math.abs(nextDt) < this.dtMinMag)
  	nextDt = this.dtMinMag * (nextDt > 0 ? 1: -1);
  if (this.dtMaxMag !== undefined && Math.abs(nextDt) > this.dtMaxMag)
  	nextDt = this.dtMaxMag * (nextDt > 0 ? 1: -1);
