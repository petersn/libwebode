...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "template.js")) as f:
    JS_TEMPLATE = f.read()
...
class Context:
    def __init__(self, external_compilation_parameters=None):
        ...
        self.compilation_id = next_compilation_id()
    ...
    def codegen_numba_expr(self, expr):
        # XXX: FIXME: Horrible hack for now.
        s = self.codegen_js_expr(expr).replace("Math.", "np.")
        s = s.replace("np.max", "np.maximum")
        s = s.replace("np.min", "np.minimum")
        # Ugh, np.pow isn't a real function.
        # TODO: Actually just do this properly.
        s = re.sub(r"/\*.*?\*/", "", s)
        s = re.sub(r"state\[", "state[samp, ", s)
        s = re.sub(r"parameters\[", "parameters[samp, ", s)
        return s

    def codegen_c_expr(self, expr):
        if isinstance(expr, (StateVariable, AdjustableParameter, CompileTimeData)):
            return self.codegen_js_expr(expr)
        elif isinstance(expr, Expr):
            args = [self.codegen_c_expr(arg) for arg in expr.args]
            if expr.op == "globalTime":
                return "t"
            if expr.op == "globalStepSize":
                return "dt"
            if expr.op == "WienerDerivativeUnstable":
                return "wiener_derivative_unstable()"
            if expr.op == "RealizedProcess":
                rp_name = self.realized_processes[expr.realized_index][0]
                return "%s(realized_processes + %i * REALIZED_PROCESS_LENGTH, t)" % (
                    {
                        "PoissonProcess": "fetch_lerp",
                        "WienerProcess": "fetch_lerp",
                        "WienerDerivative": "fetch_floor",
                    }[rp_name],
                    expr.realized_index,
                )
            fn_table = {
                ...
                "ceil": "ceil",
                "round": "round",
                # Ugh, we only have this in a binary form.
                # TODO: Properly expand the variadic form.
                "min": "fmin",
                "max": "fmax",
                "%": "fmod",
                "^": "pow",
            ...
            if expr.op in fn_table:
                ...
                if len(args) == 2:
                    return "(%s %s %s)" % (args[0], expr.op, args[1])
        self.interpreter_error("Bug: Unhandled expr in C codegen: %r" % (expr,))
    ...
    def codegen_native(self, backend):
        subst = self.codegen_js()
        subst["allocate_code"] += "\n" + """
            return new Promise(resolve => {
                ctx.wsRequestQueue = [];
                ctx.websocket = new WebSocket("ws://" + SERVER_HOST + "/computation");
                ctx.websocket.onopen = () => {
                    console.log("WebSocket connected.");
                    resolve(ctx);
                };
                ctx.websocket.onmessage = (event) => {
                    if (ctx.wsRequestQueue.length > 0) {
                        ctx.wsRequestQueue.shift()(event);
                    } else {
                        alert("Inexplicable message from server: " + event.data);
                    }
                };
                var nextRequestId = 0;
                ctx.wsRequest = (data) => {
                    const myRequestId = nextRequestId++;
                    data.requestId = myRequestId;
                    ctx.websocket.send(JSON.stringify(data));
                    return new Promise((resolve) => {
                        ctx.wsRequestQueue.push((event) => {
                            const responseObject = JSON.parse(event.data);
                            if (responseObject.requestId !== myRequestId) {
                                alert("BUG BUG BUG: Got back response for request " + responseObject.requestId
                                    + " when " + myRequestId + " was expected.");
                            }
        ...
        """
        # Erase some useless code.
        for name in ["derivative_code", "objective_code"]:
            subst[name] = ""

        self.native = backend(self)
        return subst

    def native_backend_fulfill_request(self, request):
        assert self.settings["backend"] == self.native.BACKEND_NAME
        return self.native.fulfill_request(request)

    def codegen(self):
        self.codegen_shared()
        if self.settings["backend"] == "js":
            subst = self.codegen_js()
        elif self.settings["backend"] == "native-numba":
            import native_numba_backend
            subst = self.codegen_native(native_numba_backend.NumbaBackend)
        elif self.settings["backend"] == "native-c":
            import native_c_backend
            subst = self.codegen_native(native_c_backend.CBackend)
        else:
            self.interpreter_error("Unknown backend: %s (must be one of js, native-numba, native-c)" % self.settings["backend"])
        return JS_TEMPLATE % subst
